function get_optimal_parameters(analysis_name)
...
%                     FvFm_exp);
toc
options = optimoptions('ga', 'Display', 'iter',  'PlotFcn', {@optimplotfval});

[xopt,fval,exitflag,output,population,scores] = ga(@(x) calc_sqerror(x,...Set of parameters including k and yinitia
                    n_trains, n_flashes, flash_duration, flash_interval, train_interval, ... Experimental parameters
                    ...
                    kconst, rate_inds, S, species, knames, species_idcs,...
                    FvFm_exp), ...
                    length(x0), [], [], Aeq, beq, lb, ub, [], options);
...
save([analysis_name, '/result_',datestr(now, 30)], 'xopt', 'fval', 'exitflag', 'output', 'population', 'scores',...optimization parameters
    'n_trains', 'n_flashes', 'flash_duration', 'flash_interval', 'train_interval', ... Experimental parameters
                    'kidcs', 'PSIidcs',... all indices needed in to calculate FvFm and prepare the variables
                    'tablek', 'tabley',... information on the k and y variables
                    'kconst', 'rate_inds', 'S', 'species', 'knames', 'species_idcs',...
