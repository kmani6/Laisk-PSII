function dydt = LaiskODENew (t,y,k,knames,Ynames) 
...
dydt(YoPoAo) = -k(jd)*y(YoPoAo)...
    +k(oqd)*y(YoPoAoBoo)...
    +k(rqd)*y(YoPoAoBrr)...
    -(k(oqr)*y(PQ)+k(rqr)*y(PQH2))*y(YoPoAo); 

dydt(YoPoAoBoo) = -k(jd)*y(YoPoAoBoo)...
    -k(oqd)*y(YoPoAoBoo)...
    +k(oqr)*y(PQ)*y(YoPoAo); 

	
dydt(YoPrAo) = k(jd)*y(YoPoAo) - k(jd)*y(YoPrAo)... 
+ k(rqd)* y(YoPrAoBrr) + k(oqd)*y(YoPrAoBoo) - (k(oqr)*y(PQ)+k(rqr)*y(PQH2))*y(YoPrAo)... 
- n2*k(kp)/(1+k(kp)+k(kn)+k(kr))*y(YoPrAo);

dydt(YoPoAr) = -k(jd)*y(YoPoAr)...
    +(n2)*k(kp)/(1+k(kp)+k(kn)+k(kr))*y(YoPrAo)...
    +k(rqd)*y(YoPoArBrr)...
    +k(oqd)*y(YoPoArBoo)...
    -(k(oqr)*y(PQ)+k(rqr)*y(PQH2))*y(YoPoAr);

dydt(YoPrAoBoo) = -k(jd)*y(YoPrAoBoo)... 
    + k(jd)*y(YoPoAoBoo)... 
    - n2*kp/(1+k(kp)+k(kn)+k(kr))*y(YoPrAoBoo)...
    -k(oqd)*y(YoPrAoBoo)...
    +k(oqr)*y(PQ)*y(YoPrAo);		

dydt(YoPoArBoo) = -k(jd)*y(YoPoArBoo)
    +n2*k(kp)/(1+k(kp)+k(kn)+k(kr))*y(YoPrAoBoo)...
    +k(oqr)*y(PQ)*y(YoPoAr)...
    -k(oqd)*y(YoPoArBoo)...
    -k(b1d)*y(YoPoArBoo)...
    +k(b1r)*y(YoPoAoBro);
...
dydt(YoPoAoBro) = -k(jd)*y(YoPoAoBro) ...
    + k(b1d)*y(YoPoArBoo)...
    -k(b1r)*y(YoPoAoBro);						

dydt(YrPrAo) = k(jd)*y(YoPrAo)...
    -n2*(k(kp)/(1+k(kp)+k(kn)))*y(YrPrAo)...
    +k(rqd)*y(YrPrAoBrr)...
    +k(oqd)*y(YrPrAoBoo)...
    -(k(oqr)*y(PQ)+k(rqr)*y(PQH2))*y(YrPrAo);		


dydt(YoPrAr) = k(jd)*y(YoPoAr)...
    -k(jd)*y(YoPrAr)...
    +n2*k(kp)/(1+k(kp)+k(kn))*y(YrPrAo)...
    +k(rqd)*y(YoPrArBrr)...
    +k(oqd)*y(YoPrArBoo)...
    -(k(oqr)*y(PQ)+k(rqr)*y(PQH2))*y(YoPrAr);		

dydt(YrPrAoBoo) = k(jd)*y(YoPrAoBoo)...
    -n2*k(kp)/(1+k(kp)+k(kn))*y(YrPrAoBoo)...
    +k(oqr)*y(PQ)*y(YrPrAo)...
    -k(oqd)*y(YrPrAoBoo); 	

dydt(YoPrArBoo) = k(jd)*y(YoPoArBoo)...
    -k(jd)*y(YoPrArBoo)...
    +n2*k(kp)/(1+k(kp)+k(kn))*y(YrPrAoBoo)...
    -k(b1d)*y(YoPrArBoo)...
    +k(b1r)*y(YoPrAoBro)...
...
dydt(YoPrAoBro) = k(jd)*y(YoPoAoBro)...
    -k(jd)*y(YoPrAoBro)...
    -n2*k(kp)/(1+k(kp)+k(kn)+k(kr))*y(YoPrAoBro)...
    +k(b1d)*y(YoPrArBoo)...
    -k(b1r)*y(YoPrAoBro);		

dydt(YoPoAoBrr) = -k(jd)*y(YoPoAoBrr)...
    +k(b2d)*y(YoPoArBro)...
    +k(rqr)*y(PQH2)*y(YoPoAo)...
    -k(rqd)*y(YoPoAoBrr);		 		

dydt(YrPrAr) = k(jd)*y(YoPrAr)...
    +k(rqd)*y(YrPrArBrr)...
    +k(oqd)*y(YrPrArBoo)...
    -(k(oqr)*y(PQ)+k(rqr)*y(PQH2))*y(YrPrAr);

dydt(YrPrArBoo) = k(jd)*y(YoPrArBoo)...
    -k(b1d)*y(YrPrArBoo)...
    +k(b1r)*y(YrPrAoBro)...
    +k(oqr)*y(PQ)*y(YrPrAr)...
    -k(oqd)*y(YrPrArBoo);		

dydt(YoPrArBro) = k(jd)*y(YoPoArBro)...
    -k(jd)*y(YoPrArBro)...
    +n2*k(kp)/(1+k(kp)+k(kn))*y(YrPrAoBro)...
    -k(b2d)*y(YoPrArBro);		

dydt(YoPrAoBrr) = k(jd)*y(YoPoAoBrr)...
    -k(jd)*y(YoPrAoBrr)...
    -n2*k(kp)/(1+k(kp)+k(kn)+k(kr))*y(YoPrAoBrr)...
    +k(b2d)*y(YoPrArBro)...
    -k(rqd)*y(YoPrAoBrr)...
    +k(rqr)*y(PQH2)*y(YoPrAo);		

dydt(YrPrAoBro) = k(jd)*y(YoPrAoBro)...
-n2*k(kp)/(1+k(kp)+k(kn))*y(YrPrAoBro)...
+k(b1d)*y(YrPrArBoo)...
-k(b1r)*y(YrPrAoBro);		

dydt(YoPoArBrr) = -k(jd)*y(YoPoArBrr)...
+n2*k(kp)/(1+k(kp)+k(kn)+k(kr))*y(YoPrAoBrr)...
-k(rqd)*y(YoPoArBrr)...
+k(rqr)*y(PQH2)*y(YoPoAr);	

dydt(YrPrArBro) = k(jd)*y(YoPrArBro)...
-k(b2d)*y(YrPrArBro);	

dydt(YrPrAoBrr) = k(jd)*y(YoPrAoBrr)...
-n2*k(kp)/(1+k(kp)+k(kn))*y(YrPrAoBrr)...
+k(b2d)*y(YrPrArBro)...
-k(rqd)*y(YrPrAoBrr)...
+k(rqr)*y(PQH2)*y(YrPrAo);	

dydt(YoPrArBrr) = k(jd)*y(YoPoArBrr)...
-k(jd)*y(YoPrArBrr)...
+n2*k(kp)/(1+k(kp)+k(kn))*y(YrPrAoBrr)...
-k(rqd)*y(YoPrArBrr)...
+k(rqr)*y(PQH2)*y(YoPrAr);				

dydt(YrPrArBrr) = k(jd)*y(YoPrArBrr)...
-k(rqd)*y(YrPrArBrr)...
+k(rqr)*y(PQH2)*y(YrPrAr);	

dydt(PQH2) = k(rqd)*(y(YoPoAoBrr)+y(YoPrAoBrr)+ y(YoPoArBrr)+ y(YrPrAoBrr)+ y(YoPrArBrr)+ y(YrPrArBrr))...
-k(rqr)*y(PQH2)*(y(YoPoAo)+y(YoPrAo)+y(YoPoAr)+y(YrPrAo)+y(YoPrAr)+y(YrPrAr))...
- (Vb6f);

dydt(PQ) = k(oqd)*(y(YoPoAoBoo)+y(YoPrAoBoo)+y(YoPoArBoo)+y(YrPrAoBoo)+y(YoPrArBoo)+y(YrPrArBoo))...
-k(oqr)*y(PQ)*(y(YoPoAo)+y(YoPrAo)+y(YoPoAr)+y(YrPrAo)+y(YoPrAr)+y(YrPrAr))...
+Vb6f;

dydt(Cytfr) = 2*Vb6f...
-k(kcytf)*(y(Cytfr)*y(PCo)-y(Cytfo)*(y(PCr)/k(kEcytf)));

dydt(PCr) = k(kcytf)*(y(Cytfr)*y(PCo)- y(Cytfo)*y(PCr)/k(kEcytf))...
-k(kpc)*(y(PCr)*y(P700o)-y(PCo)*(y(P700r)/k(kEpc)));

dydt(P700r) = k(kpc)*(y(PCr)*y(P700o)-y(PCo)*(y(P700r)/kEpc))...
-n1*y(P700r)*y(FXo);

dydt(FXr) = n1*y(P700r)*y(FXo)...
-k(kfx)*(y(FXr)*y(FDo)-y(FXo)*(y(FDr)/k(kEfx)));

dydt(FDr) = k(kfx)*(y(FXr)*y(FDo)-y(FXo)*(y(FDr)/k(kEfx)))...
-k(kfd)*y(FDr);

dydt(O2) = k(jd)*(y(YoPoAo)+y(YoPoAoBoo)+y(YoPrAo)+y(YoPoAr)+y(YoPrAoBoo)+y(YoPoArBoo)...
+y(YoPoAoBro)+y(YoPrAr)+y(YoPrArBoo)+y(YoPrAoBro)+y(YoPoArBro)+y(YoPoAoBrr)+y(YoPrArBro)...
+y(YoPrAoBrr)+y(YoPoArBrr)+y(YoPrArBrr));
