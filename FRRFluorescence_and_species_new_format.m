function [ts,ys, Fs, FvFm_sim, species] = FRRFluorescence_and_species_new_format(analysis_name)
...
[ts,ys, Fs, FvFm_sim] = calc_Species_concs(x0,... Set of parameters. This only includes the independent variables as described by the third column in Y and Constants files
                    n_trains, n_flashes, flash_duration, flash_interval, train_interval, ... Experimental parameters
                    kidcs, PSIidcs, ... all indices needed in to calculate FvFm and prepare the variables
                    tablek, tabley,... information on the k and y variables
                    kconst, rate_inds, S, species, knames, species_idcs); % model specific variables  
...
fig_dir = [analysis_name, '/figures', datestr(now,30)];
mkdir(fig_dir)
...
ylabel('Fq/Fm')
xlabel('STF')
savefig(f_fvfm_raw, [fig_dir, '/fvfmraw.fig'], 'compact')
saveas(f_fvfm_raw, [fig_dir, '/fvfmraw.png'])


fm = reshape(FvFm_sim,50,[]);
a = mean(fm,1);
f_fvfm_avg = figure; plot(1:length(a), a,'.-')
ylabel('Average Fq/Fm per train')
xlabel('train number')
savefig(f_fvfm_avg, [fig_dir, '/fvfmavg.fig'], 'compact')
saveas(f_fvfm_avg, [fig_dir, '/fvfmavg.png'])

f_pq_redox = plot_pq_redox_state(species, ys, ts);
savefig(f_pq_redox, [fig_dir, '/PQ.fig'], 'compact')
saveas(f_pq_redox, [fig_dir, '/PQ.png'])

f_s_states = plot_S_states(species, ys, ts);
savefig(f_s_states, [fig_dir, '/s_states.fig'], 'compact')
saveas(f_s_states, [fig_dir, '/s_states.png'])

f_fd_redox = plot_fd_redox_state(species, ys, ts);
savefig(f_fd_redox, [fig_dir, '/fd.fig'], 'compact')
saveas(f_fd_redox, [fig_dir, '/fd.png'])

f_pc_redox = plot_pc_redox_state(species, ys, ts);
savefig(f_pc_redox, [fig_dir, '/pc.fig'], 'compact')
saveas(f_pc_redox, [fig_dir, '/pc.png'])

f_nad_redox = plot_NAD_redox_state(species, ys, ts);
savefig(f_nad_redox, [fig_dir, '/nadp.fig'], 'compact')
saveas(f_nad_redox, [fig_dir, '/nadp.png'])

f_H_species = plot_H_species(species, ys, ts);
savefig(f_H_species, [fig_dir, '/H.fig'], 'compact')
saveas(f_H_species, [fig_dir, '/H.png'])

f_atp = plot_atp_species(species, ys, ts);
savefig(f_atp, [fig_dir, '/atp.fig'], 'compact')
saveas(f_atp, [fig_dir, '/atp.png'])
