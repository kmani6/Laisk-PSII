...
y(yidcs.deltapsiindex) = deltapsi_ss;
...
D = 1 + x + x^2 + x^3 + x^4; 
p1 = (ATPpar.kFC*x^4)/((1+ATPpar.kFC)); 
p2 = 1/((1+ATPpar.kFC)*D); 
...
dydt(yidcs.deltapsiindex) = (dydt(yidcs.Hl)-dydt(yidcs.Hs))* ATPpar.Vlumen/ATPpar.thylac*ATPpar.F/ATPpar.Cmem+ v_counter;%(((VHin-VHout)/ATPpar.Sth + Fi)*ATPpar.F)/ATPpar.Cmem; %*1e-4;
