function [ts,ys, Fs, FvFm, species, FvFmErr, XErr,yidcs,ATPpar,kf1indcs, kf2indcs] = nonlcon_optimal_parameters1(analysis_name,randomseed)
...
sqr_initial = calc_sqerror1(x0,...Set of parameters including k and yinitial
                    ...
                    kconst, rate_inds, S, species, knames, species_idcs,FvFm_exp,...
                    yidcs, ATPpar,kf1indcs, kf2in0dcs); 
                

options = optimset('Display', 'iter', 'GradObj', 'off', 'Algorithm', 'interior-point',...
    'MaxFunEvals', 10000, 'PlotFcn', {@optimplotfval});

[xopt, fval, exitflag, output] = fmincon(@(x) calc_sqerror1(x,...Set of parameters including k and yinitial
                    n_trains, n_flashes, flash_duration, flash_interval, train_interval, ... Experimental parameters
                    Fluorescence_k_idcs, Fluorescence_y_inds,... Indeces to calculate Fluorescence
                    kidcs, PSIidcs, ... all indices needed to calculate FvFm and prepare the variables
                    tablek, tabley,... information on the k and y variables
                    ...
                    x0, [], [], Aeq, beq, lb, ub, @(x) nonlinconstr(x,kwfindep,kwbindep), options);
                
FvFm_sim_opt = calc_FvFm1(xopt,... Set of parameters. This only includes the independent variables as described by the third column in Y and Constants files
                    n_trains, n_flashes, flash_duration, flash_interval, train_interval, ... Experimental parameters
                    Fluorescence_k_idcs, Fluorescence_y_inds,... Indeces to calculate Fluorescence
                    kidcs, PSIidcs, ... all indices needed in to calculate FvFm and prepare the variables
                    tablek, tabley,... information on the k and y variables
                    kconst, rate_inds, S, species, knames, species_idcs,kf2index,kf2indcs); % model specific variables
...
save(['results/', analysis_name '/optimal' datestr(now, 30)],...
